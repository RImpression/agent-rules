...
生成规范后，主流 AI 编码助手会自动识别：

- **Cursor / Windsurf** — 自动读取 `AGENTS.md` 和 `.agents/` 中的规则
- **GitHub Copilot** — 通过 `AGENTS.md` 获取项目上下文
- **Claude (Anthropic)** — 识别 `CLAUDE.md` 转发入口
- **Gemini (Google)** — 识别 `GEMINI.md` 转发入口

建议在 `AGENTS.md` 同级创建转发文件：

```markdown
<!-- CLAUDE.md -->
请阅读 AGENTS.md 获取本项目的完整规范。
```
